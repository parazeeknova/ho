...
describe("classifyWorkdayControl", () => {
  it("classifies comboboxes by role or aria-autocomplete", () => {
    assert.equal(classifyWorkdayControl({ tag: "INPUT", type: "text", role: "combobox" }), "combobox");
    assert.equal(classifyWorkdayControl({ tag: "INPUT", type: "text", ariaAutocomplete: true }), "combobox");
  });
