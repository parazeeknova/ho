...
async def _ensure_torproxy() -> None:
    ...
    try:
        proc = await asyncio.create_subprocess_exec(
            "docker", "compose", "-f", str(_PROJECT_ROOT / "docker-compose.yaml"),
            "up", "-d", "torproxy",
...
async def _read_tor_cookie_hex() -> str:
    ...
    try:
        ...
        if not container:
            return ""
        proc = await asyncio.create_subprocess_exec(
            "docker", "exec", container, "cat", "/etc/tor/run/control.authcookie",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.DEVNULL,
        )
        out, _ = await proc.communicate()
...
async def _rotate_tor_circuit() -> None:
    ...
    try:
        ...
        auth = (await asyncio.wait_for(reader.read(256), timeout=3.0)).decode(
            errors="replace"
        )
...
class AutofillWorker:
        ...
        async with self._profile_lock:
            for p in self._profile_pool:
                if p not in self._profile_in_use:
                    for name in ("SingletonLock", "SingletonSocket", "SingletonCookie", "chrome.pid"):
                        stale = Path(p) / name
    ...
    async def send_end_of_run_summary(self) -> None:
        ...
        text = self._format_digest(
            rows, title="🏁 Overnight run finished — deferred jobs"
        )
        ok = await self._send_chunked(bridge, text)
        if ok:
    ...
    async def _send_chunked(
        bridge: "TelegramQuestionBridge", text: str, max_len: int = 3900
    ) -> bool:
