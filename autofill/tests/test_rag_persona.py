...
async def test_residence_composite_yes_no_only_answers_residence_facet():
    ...
    assert (
        answers[
            "Are you currently based in Europe (or willing to work from a European timezone long-term)?"
        ]
        == "No"
    )
    mock_cm.chat.assert_not_called()
...
async def test_office_question_ignores_willingness_phrasing():
    ...
    mock_cm.chat = AsyncMock(
        return_value=json.dumps(
            {
                "We have an all-day, in-office policy. Are you willing to work onsite, either in Paris or Tel Aviv?": "Yes"
            }
        )
    )
    with _embed_stub():
    ...
    assert (
        answers[
            "We have an all-day, in-office policy. Are you willing to work onsite, either in Paris or Tel Aviv?"
        ]
        == "Yes"
    )
...
async def test_position_statement_is_not_a_residence_question():
    ...
    assert (
        answers[
            "This position is based in Bangkok, Thailand and requires "
