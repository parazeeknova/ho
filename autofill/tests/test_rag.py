...
async def test_expected_comp_answers_in_question_currency():
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        assert (
            await rag.kb_answer("What is your expected salary in USD?")
            == "$8,300"
        )
        assert (
            await rag.kb_answer("What is your expected annual salary in EUR?")
            == "€65,000"
        )
        # Unspecified granularity defaults to monthly (matches the INR persona).
        assert (
            await rag.kb_answer("What are your salary expectations in GBP?")
            == "£4,600"
        )
        # No currency named and no job country: unchanged INR behavior.
        assert (
            await rag.kb_answer("What is your expected salary?")
            == "80K INR/month"
        )
...
async def test_expected_comp_currency_overrides_inr_custom_answer():
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        assert (
            await rag.kb_answer("What is your expected compensation in USD?")
            == "$8,300"
        )
        # The exact INR question still answers INR.
        assert (
            await rag.kb_answer("What is your expected compensation?")
            == "80000 INR/ month"
        )
...
async def test_proficiency_gate_uses_resume_skill_whitelist():
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        # Resume skills -> Yes.
        assert (
            await rag.kb_answer("Do you have experience with React?") == "Yes"
        )
        assert (
            await rag.kb_answer("Are you comfortable with TypeScript?") == "Yes"
        )
        assert (
            await rag.kb_answer("Do you have strong proficiency in both Python and Rust?")
            == "Yes"
        )
        # "one of (Go, React, TypeScript)" -> Yes because React/TS are on the resume.
        assert (
            await rag.kb_answer(
                "Do you have proficiency in one of our primary languages "
                "(Go, React, TypeScript)?"
            )
            == "Yes"
        )
        ...
        assert (
            await rag.kb_answer("Do you have experience with Kubernetes?") == "No"
        )
        assert (
            await rag.kb_answer("Do you have experience with Docker?") == "No"
        )
...
async def test_proficiency_gate_ignores_non_skill_phrasing():
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        answers = await rag.answer_questions(
            ["Do you have experience building real-time systems?"]
        )
    assert answers["Do you have experience building real-time systems?"] == (
        "Yes, I built Larity"
    )
...
async def test_proficiency_gate_non_tech_and_numeric_questions_left_to_llm():
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        assert (
            await rag.kb_answer("Do you have experience with leadership?") is None
        )
        assert (
            await rag.kb_answer("Do you have experience with team management?") is None
        )
        assert (
            await rag.kb_answer("Are you familiar with our company culture?") is None
        )
        assert (
            await rag.kb_answer("Do you have any experience with the insurance industry?")
            is None
        )
        assert (
            await rag.kb_answer("How many years of experience with React do you have?")
            is None
        )
        assert (
            await rag.kb_answer("What are your years of experience in React?") is None
        )
...
async def test_proficiency_gate_or_and_multiword_and_such_as():
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        assert (
            await rag.kb_answer("Do you have experience with Vue or React?") == "Yes"
        )
        assert (
            await rag.kb_answer("Do you have experience with React or Vue?") == "Yes"
        )
        assert (
            await rag.kb_answer("Do you have experience with REST APIs?") == "Yes"
        )
        assert (
            await rag.kb_answer("Do you have experience with Vercel AI SDK?") == "Yes"
        )
        assert (
            await rag.kb_answer(
        ...
        assert (
            await rag.kb_answer(
                "Do you have experience with both React and Node (in production)?"
            )
            == "Yes"
        )
        # Both named, but Kubernetes is not on the resume -> No.
        assert (
            await rag.kb_answer("Do you have experience with both Kubernetes and Docker?")
            == "No"
        )
...
async def test_expected_comp_foreign_currency_without_entry_never_leaks_inr():
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        ...
        assert (
            await rag.kb_answer("What is your expected salary in JPY?") is None
        )
        # Covered currencies still resolve.
        assert (
            await rag.kb_answer("What is your expected salary in EUR?") == "€5,400"
        )
...
async def test_expected_comp_symbol_currency_ordering():
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        assert (
            await rag.kb_answer("What is your expected salary in C$?") == "C$7,900"
        )
        assert (
            await rag.kb_answer("What is your expected salary in A$?") == "A$8,300"
        )
        assert (
            await rag.kb_answer("What is your expected salary in S$?") == "S$6,700"
        )
...
async def test_generate_cover_letter_strips_em_dashes():
    ...
    rag = ScreenerRAG(
        ...
        exact_answers={},
        profile=MagicMock(
            firstName="Aman", lastName="Aziz", email="a@b.com",
    ...
    assert "\u2014" not in text
    assert "I built X, and it worked." == text
...
def test_resolve_authorization_policy_home_country() -> None:
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        rag = ScreenerRAG(
            exact_answers={}, scoped_answers={}, profile=profile, store=None
        )
...
def test_target_country_residence_phrasing_uses_home_country() -> None:
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        ...
        assert (
            rag.target_country(
                "Are you authorized to work in your home country?", job
            )
            == "india"
        )
...
async def test_persona_scoped_fact_never_answers_unknown_country() -> None:
    ...
    with patch("autofill.rag._embed_text", new=AsyncMock(return_value=[0.1])):
        assert (
            await rag.kb_answer("Are you authorized to work in the country?") is None
        )
...
async def test_generate_cover_letter_grounds_on_resume_and_company() -> None:
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()), patch(
        "autofill.rag._embed_text", AsyncMock(return_value=[0.1, 0.2])
    ):
        text = await rag.generate_cover_letter(
            job_context={
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Do you know, or are you related to, anyone who currently works at "
            "Dominion Dynamics?",
            ["Yes", "No"],
            "No",
        ),
        (
            "Have you ever been employed by Anduril or any company that Anduril "
            "has acquired?",
            ["Yes", "No"],
            "No",
        ),
...
async def test_residence_composite_free_text_no_yes_fabrication() -> None:
    ...
    with patch("autofill.rag.get_config", return_value=_cfg()):
        ...
        assert rag.resolve_residence_policy(
