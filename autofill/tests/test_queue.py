...
async def test_fill_ttl_expires_and_purges(db: AutofillDB):
    ...
    async with db._pool.acquire() as conn:
        await conn.execute(
            "UPDATE autofill_fills SET expires_at = NOW() - INTERVAL '1 second' WHERE question = 'Q1'"
        )
    ...
    async with db._pool.acquire() as conn:
        await conn.execute(
            "UPDATE autofill_fills SET expires_at = NOW() - INTERVAL '1 second' WHERE question = 'Q2'"
        )
