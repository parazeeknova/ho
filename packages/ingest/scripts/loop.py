    ...
    try:
        while True:
            ...
            if radar_done and balance["open"] == 0:
                ...
                break
            if not radar_done:
                # target_submissions <= 0 means NO upper bound: keep discovering
                # and filing applications until the queue goes idle, --max-minutes
                # elapses, or the user Ctrl+C's. A positive target stops the run
                # once that many CONFIRMED applications are reached for the
                # current epoch.
                epoch_open = 0
                epoch_target = 0
                try:
                    ...
                    pass
                if epoch_target > 0:
                    # A work-session epoch below its target: a quiet bridge
                    # cycle does not mean the run is finished, it means
                    # discovery is still filling the reservoir.
                    if epoch_open < epoch_target:
                        if enqueued > 0 or balance["open"] > 0:
                            idle_cycles = 0
                        continue
                    # Epoch target reached: the run's goal is met. Mark the
                    # radar done so we only drain whatever remains, then exit.
                    if epoch_open >= epoch_target:
