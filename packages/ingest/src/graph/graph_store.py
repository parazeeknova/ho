...
class GraphStore:
    ...
    async def get_nodes_by_type(self, node_type: NodeType, limit: int = 100) -> list[GraphNode]:
        rows = await self._run(
            "MATCH (n:GraphNode {node_type: $t, active: true}) RETURN n "
            "ORDER BY n.updated_at DESC LIMIT $l",
    ...
    async def search_companies(self, query: str, limit: int = 20) -> list[GraphNode]:
        rows = await self._run(
            ...
            "WHERE n.data CONTAINS $q RETURN n ORDER BY n.updated_at DESC LIMIT $l",
            {"q": query, "l": limit},
        )
        return [self._row_to_node(_unpack(r, "n")) for r in rows]
    ...
    async def decay_stale_confidence(self, max_age_days: int = 30) -> int:
        rows = await self._run(
            "MATCH (n:GraphNode {active: true}) "
            "WHERE datetime(n.last_verified) < datetime() - duration({days: $d}) RETURN n",
            {"d": max_age_days},
        )
        updated = 0
        for r in rows:
    ...
    async def upsert_edge(self, edge: GraphEdge) -> tuple[GraphEdge, list[MutationEvent]]:
        ...
        logger.info(
            f"Graph edge created: ({edge.source_id}) "
    ...
    async def get_edge(
        ...
        rows = await self._run(
            "MATCH (a:GraphNode {id: $s})-[r:RELATES {type: $t}]->(b:GraphNode {id: $tg}) "
            "RETURN a.id, r, b.id",
    ...
    async def get_neighbors(self, node_id: str) -> list[tuple[GraphEdge, GraphNode]]:
        rows = await self._run(
            "MATCH (a:GraphNode {id: $id})-[r:RELATES]->(b:GraphNode) "
            "WHERE b.active = true RETURN a.id, r, b",
            {"id": node_id},
        )
        return [
            (
                ...
                signals.append(
                    {
                        ...
                        "weight": 0.12,
                        "description": (
                            f"Structurally similar to {len(hiring_similar)} active hiring companies"
                        ),
        ...
        try:
            rows = await self._run(
                ...
                MATCH (v:GraphNode {node_type: 'investor'})
                     <-[:RELATES {type: 'invested_by'}]-(f:GraphNode)
                     -[:RELATES {type: 'has_funding'}]-(c:GraphNode)
                     -[:RELATES {type: 'posted_job'}]->(j:GraphNode)
    ...
    async def find_warm_intro_paths(
        ...
        try:
            rows = await self._run(
                """
                MATCH (me:GraphNode {id: $candidate})-[:RELATES]->(shared)
                      <-[:RELATES]-(f:GraphNode {node_type: 'founder'})
