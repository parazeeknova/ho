...
class AutofillDB:
    ...
    async def release_stale_leases(
        self, stale_minutes: int = 30, reset_all_filling: bool = False
    ) -> int:
        ...
        When ``reset_all_filling`` is True (single-worker boot reclaim), every
        ``filling`` row is reset regardless of age — the previous worker is
        gone, so its in-flight jobs are orphans and must be re-processed.
        """
        async with self._pool.acquire() as conn:
            if reset_all_filling:
