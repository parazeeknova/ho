...
from autofill.src.screener.rag import ASK_USER, ScreenerRAG, _pick_authorization_answer
...
def _normalise_question_import(s):
    from autofill.src.screener.rag import _normalise_question
...
def test_authorization_classifies_uk_spelling_and_variants() -> None:
    ...
    from autofill.src.screener.rag import is_scoped_question
...
async def test_learn_scoped_qualifies_question_text(tmp_path) -> None:
    ...
    with (
        patch("autofill.src.screener.rag.PERSONA_JSON", persona_json),
        patch("autofill.src.screener.rag.PERSONA_TXT", persona_txt),
    ):
        ok = await rag.learn(
...
async def test_persona_scoped_fact_never_answers_unknown_country() -> None:
    ...
    with patch("autofill.src.screener.rag._embed_text", new=AsyncMock(return_value=[0.1])):
...
def test_normalize_start_date_fixes_typo_and_relative_offsets() -> None:
    from autofill.src.screener.rag import _normalize_start_date
...
def test_start_date_question_is_recognized() -> None:
    from autofill.src.screener.rag import _PERSONAL_RULES
...
async def test_generate_cover_letter_grounds_on_resume_and_company() -> None:
    ...
    with (
        patch("autofill.src.screener.rag.get_config", return_value=_cfg()),
        patch("autofill.src.screener.rag._embed_text", AsyncMock(return_value=[0.1, 0.2])),
    ):
        text = await rag.generate_cover_letter(
            job_context={
...
async def test_gather_context_unions_queries_and_drops_noise() -> None:
    ...
    with patch("autofill.src.screener.rag._embed_text", AsyncMock(return_value=[0.1, 0.2])):
...
def test_country_from_location(location, expected) -> None:
    from autofill.src.screener.rag import _country_from_location
