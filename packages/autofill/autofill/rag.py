...
class ScreenerRAG:
    ...
    def resolve_visa_policy(
        ...
        """
        matched = next(((p, k) for p, k in _PERSONAL_RULES if p.search(question or "")), None)
...
"""
        ...
        try:
            return (await self.cm.chat(prompt, system_prompt=COVER_LETTER_SYSTEM_PROMPT)).strip()
        except Exception as e:
